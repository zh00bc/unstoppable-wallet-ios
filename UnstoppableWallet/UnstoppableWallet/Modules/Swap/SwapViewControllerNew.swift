...
//    private let recipientCell = AdditionalDataCellNew()
    ...
    override func viewDidLoad() {
        ...
        poweredByView.title = "Powered by DEXNAME!!!" //\(viewModel.dexName)"
...
//        recipientCell.title = "swap.advanced_settings.recipient_address".localized
    ...
    private func subscribeToViewModel() {
        ...
        subscribe(disposeBag, viewModel.additionTradeInfoDriver) { [weak self] in self?.handle(additionTradeViewItems: $0) }
//        subscribe(disposeBag, viewModel.tradeOptionsViewItemDriver) { [weak self] in self?.handle(tradeOptionsViewItem: $0) }
    ...
    private func handle(additionTradeViewItems: [SwapViewModelNew.AdditionalTradeInfoViewItem]) {
...
//        }
        ...
        poweredByView.isHidden = !additionTradeViewItems.isEmpty
...
//        reloadTable()
    ...
    private func handle(proceedActionState: SwapViewModelNew.ActionState) {
        handle(actionState: proceedActionState, button: proceedButton)
    }

    private func handle(approveActionState: SwapViewModelNew.ActionState) {
        handle(actionState: approveActionState, button: approveButton)
    }

    private func handle(actionState: SwapViewModelNew.ActionState, button: ThemeButton) {
        switch actionState {
        case .hidden:
...
extension SwapViewControllerNew: IDynamicHeightCellDelegate {
